...
def command_info(username, detailed = False):
	...
	# url = "http://github.com/api/v2/json/issues/list/%s/*/open" % username
...
def command_submit(repo_name, username, reviewer_repo_name = None, pull_body = None, pull_title = None, submitOpenGitHub = True):
	...
	if m is not None and m.group(1) != '':
		...
		os.system('jira -a progressIssue --issue %s --step "Reassign Review Request" --assignee "brian.chan" --field "customfield_10421" --values "%s"' % (ticket_id, pull_request.get('html_url')))
